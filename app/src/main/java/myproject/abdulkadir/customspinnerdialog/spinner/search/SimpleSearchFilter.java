package myproject.abdulkadir.customspinnerdialog.spinner.search;
...
import myproject.abdulkadir.customspinnerdialog.spinner.BaseFilter;
...
public class SimpleSearchFilter<T extends Searchable> extends BaseFilter {
    ...
    protected Filter.FilterResults performFiltering(CharSequence chars) {
        ...
        for (T item : mItems) {
            if (item.getString().toLowerCase().contains(filterSeq)) {
                filteredItems.add(new Pair<>(item, filterSeq.length()));
            } else if (mCheckLCS) {
                int lcsLength = StringsHelper.lcs(item.getString(), filterSeq).length();
                if (lcsLength > item.getString().length() * mAccuracyPercentage) {
